...
class TestGeneratesKnowledgeAPI:
    ...
    async def setup_method(self):
        ...
        self.sample_relationship = {
            ...
            "relationshipId": f"generates_knowledge_{self.win_id}_{self.snippet_id}",
    ...
    async def test_create_generates_knowledge_relationship(self, mock_service):
        """Test creating a GENERATES_KNOWLEDGE relationship via API."""
        # Setup mock properly for sync method
        mock_service.create_relationship.return_value = Relationship(**self.sample_relationship)
    ...
    @pytest.mark.asyncio
    @patch("trm_api.api.v1.endpoints.relationship.relationship_service")
    async def test_create_generates_knowledge_relationship_not_found(self, mock_service):
        """Test creating a GENERATES_KNOWLEDGE relationship with non-existent entities."""
        # Setup mock to return None (entities not found)
    ...
    @pytest.mark.asyncio
    @patch("trm_api.api.v1.endpoints.relationship.relationship_service")
    async def test_get_knowledge_snippets_by_win(self, mock_service):
        """Test getting knowledge snippets generated by a WIN."""
        # Setup mock to return list of relationships
        mock_relationship = Relationship(**self.sample_relationship)
        mock_service.get_relationships.return_value = [mock_relationship]
        ...
        # Verify mock was called correctly
        mock_service.get_relationships.assert_called_once_with(
    ...
    @pytest.mark.asyncio
    @patch("trm_api.api.v1.endpoints.relationship.relationship_service")
    async def test_get_wins_by_knowledge_snippet(self, mock_service):
    ...
    @pytest.mark.asyncio
    @patch("trm_api.api.v1.endpoints.relationship.relationship_service")
    async def test_delete_generates_knowledge_relationship(self, mock_service):
        ...
        # Verify mock was called correctly
        mock_service.delete_relationship.assert_called_once_with(
            source_id=self.win_id,
            source_type="Win",
            target_id=self.snippet_id,
            target_type="KnowledgeSnippet",
    ...
    @pytest.mark.asyncio
    @patch("trm_api.api.v1.endpoints.relationship.relationship_service")
    async def test_delete_generates_knowledge_relationship_not_found(self, mock_service):
        """Test deleting a non-existent GENERATES_KNOWLEDGE relationship."""
        # Setup mock to return False (relationship not found)
