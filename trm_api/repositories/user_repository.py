...
class UserRepository:
    ...
    def create_user(self, user_data: UserCreate) -> User:
        ...
        try:
            ...
            logger.debug(f"- Tạo User với dữ liệu: {user_dict}")
            
            # Kiểm tra xem Neo4j có kết nối được không
            from neomodel import db
            results, meta = db.cypher_query("MATCH (n) RETURN COUNT(n) LIMIT 1")
            logger.debug(f"- Kết nối Neo4j OK, số node hiện tại: {results[0][0]}")
            
            graph_user = User(**user_dict).save()
            logger.debug(f"- Đã tạo User thành công: {graph_user.username}")
            return graph_user
    ...
    def list_users(self, skip: int = 0, limit: int = 100) -> List[User]:
        ...
        """
        logger.debug(f"- UserRepository.list_users: Bắt đầu lấy danh sách người dùng. Skip: {skip}, Limit: {limit}")
        try:
            from neomodel import db
            # Kiểm tra kết nối nhanh
            db.cypher_query("MATCH (n:User) RETURN count(n) AS user_count")
            logger.debug(f"- UserRepository.list_users: Kết nối Neo4j và truy vấn kiểm tra thành công.")
            
            all_users_node_set = User.nodes.all()
            logger.debug(f"- UserRepository.list_users: Đã gọi User.nodes.all(). Số lượng (ước tính): {len(all_users_node_set)}")
            
            users = all_users_node_set[skip:skip + limit]
            logger.debug(f"- UserRepository.list_users: Đã cắt danh sách. Số lượng sau khi cắt: {len(users)}")
            
            # Chuyển đổi sang danh sách để đảm bảo không có vấn đề lazy loading
            user_list = list(users)
            logger.debug(f"- UserRepository.list_users: Hoàn thành, trả về {len(user_list)} người dùng.")
            return user_list
        except Exception as e:
            import traceback
            logger.error(f"- UserRepository.list_users: Lỗi khi lấy danh sách người dùng: {str(e)}")
            logger.error("Exception traceback: %s", traceback.format_exc())
    ...
    def get_paginated_users(self, page: int = 1, page_size: int = 10) -> Tuple[List[User], int, int]:
        ...
        """
        logger.debug(f"- UserRepository.get_paginated_users: Bắt đầu lấy danh sách người dùng phân trang. Page: {page}, Page Size: {page_size}")
        try:
            from neomodel import db
            # Kiểm tra kết nối nhanh
            results, meta = db.cypher_query("MATCH (n:User) RETURN count(n) AS user_count")
            total_count = results[0][0]
            logger.debug(f"- UserRepository.get_paginated_users: Kết nối Neo4j thành công. Tổng số người dùng: {total_count}")
            ...
            logger.debug(f"- UserRepository.get_paginated_users: Hoàn thành, trả về {len(paginated_users)} người dùng, trang {page}/{page_count}")
            return paginated_users, total_count, page_count
        except Exception as e:
            import traceback
            logger.error(f"- UserRepository.get_paginated_users: Lỗi khi lấy danh sách người dùng phân trang: {str(e)}")
            logger.error("Exception traceback: %s", traceback.format_exc())
            # Trả về danh sách rỗng trong trường hợp lỗi để tránh treo API
            return [], 0, 0
